...
class _DataAnnotatorScreenState extends State<DataAnnotatorScreen> {
    ...
    _quillController = QuillController.basic(
      config: QuillControllerConfig(
        clipboardConfig: QuillClipboardConfig(
          enableExternalRichPaste: true, // Support rich text pasting
        ),
      ),
    );
  }
    ...
    _debouncedUpdate(() {
      try {
        ...
        for (String token in tokens) {
          ...
          if (startIndex >= 0) {
            if (startIndex > currentIndex) {
              newDelta.insert(text.substring(currentIndex, startIndex));
            }
            newDelta.insert(token, {
              'background':
                  _dictionary.containsKey(token)
                      ? '#ADD8E6'
                      : _newlyAddedWords.contains(token)
        ...
        // Only update if the new Delta differs significantly
        if (!_areDeltasEqual(currentDelta, newDelta)) {
          _quillController.document = Document.fromDelta(newDelta);
        ...
        print('Error applying highlights: $e');
      }
    });
  }

  bool _areDeltasEqual(Delta a, Delta b) {
    final aJson = jsonEncode(a.toJson());
    final bJson = jsonEncode(b.toJson());
    return aJson == bJson;
  }
  ...
  Future<void> saveFile() async {
    try {
      String? defaultFileName = 'tokenized_001.txt';
      if (_currentFilePath != null) {
        // Use the loaded file name, but you can also change extension if needed
        defaultFileName = _currentFilePath!.split(Platform.pathSeparator).last;
      }
      String? outputPath = await FilePicker.platform.saveFile(
        dialogTitle: 'Save Tokenized File',
      ...
      if (outputPath != null) {
        ...
        await file.writeAsString(plainText, encoding: const Utf8Codec());
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('File saved successfully!')),
        );
        setState(() {
          _currentFilePath = outputPath; // Update the path after saving
        });
      }
    } catch (e) {
      ScaffoldMessenger.of(
